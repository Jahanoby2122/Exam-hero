    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-50 py-12 px-4">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <motion.div 
                ...
                className="card bg-white shadow-lg p-6 text-center hover:shadow-xl transition-shadow duration-300"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          {/* Left Column - Contact Info + Stats + Testimonials */}
          <div className="space-y-8">
            ...
            >
              {[
                ...
                <motion.div 
                  key={i}
                  variants={fadeIn}
                  className={`card bg-white shadow-lg border-t-4 border-${stat.color}`}
                >
            ...
            {/* Testimonials */}
            <AnimatedCard className="card bg-white shadow-xl border-t-4 border-accent">
              <div className="card-body">
                <h2 className="card-title text-2xl mb-6 flex items-center">
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div>
                  ...
                  <input 
                    ...
                    className={`input input-bordered w-full bg-gray-50 ${formErrors.name ? 'input-error' : ''}`}
                  />
                  {formErrors.name && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="label">
                      <span className="label-text font-semibold">ইমেইল ঠিকানা*</span>
                    </label>
                    <input 
                      type="email" 
                      name="email" 
                      ...
                      className={`input input-bordered w-full bg-gray-50 ${formErrors.email ? 'input-error' : ''}`}
                    />
                    {formErrors.email && (
                      <div className="flex items-center mt-1 text-error">
                        <FaInfoCircle className="mr-1 text-xs" />
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className={`input input-bordered w-full bg-gray-50 ${formErrors.phone ? 'input-error' : ''}`}
                    />
                    {formErrors.phone && (
                      <div className="flex items-center mt-1 text-error">
                        <FaInfoCircle className="mr-1 text-xs" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <label className="label">
                      <span className="label-text font-semibold">শিক্ষার্থীর শ্রেণী*</span>
                    </label>
                    <select 
                      name="studentType" 
                      value={formData.studentType} 
                      onChange={handleChange} 
                      className={`select select-bordered w-full bg-gray-50 ${formErrors.studentType ? 'select-error' : ''}`}
                    >
                    ...
                    {formErrors.studentType && (
                      <div className="flex items-center mt-1 text-error">
                        <FaInfoCircle className="mr-1 text-xs" />
                        <span className="text-sm">{formErrors.studentType}</span>
                      </div>
                    )}
                  </div>
                  <div>
                    <label className="label">
                      <span className="label-text font-semibold">বিষয় নির্বাচন*</span>
                    </label>
                    <select 
                      name="subject" 
                      value={formData.subject} 
                      onChange={handleChange} 
                      className={`select select-bordered w-full bg-gray-50 ${formErrors.subject ? 'select-error' : ''}`}
                    >
                    ...
                    {formErrors.subject && (
                      <div className="flex items-center mt-1 text-error">
                        <FaInfoCircle className="mr-1 text-xs" />
                        <span className="text-sm">{formErrors.subject}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div>
                  <label className="label">
                    <span className="label-text font-semibold">আপনার বার্তা*</span>
                  </label>
                  <textarea 
                    name="message" 
                    ...
                    className={`textarea textarea-bordered w-full bg-gray-50 h-28 ${formErrors.message ? 'textarea-error' : ''}`}
                  ></textarea>
                  {formErrors.message && (
                    <div className="flex items-center mt-1 text-error">
                      <FaInfoCircle className="mr-1 text-xs" />
        ...
        <div className="mt-20">
          ...
          <div className="space-y-4 max-w-3xl mx-auto">
            {[
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="collapse collapse-plus bg-white shadow">
                <input type="checkbox" />
        ...
        <div className="mt-20 text-center">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} className="bg-primary text-white py-10 rounded-2xl shadow-xl">
            <h2 className="text-3xl font-bold mb-4">আজই যোগ দিন</h2>
            <p className="mb-6 text-lg">সেরা শিক্ষকের কাছ থেকে পড়াশোনা শুরু করুন এবং আপনার সাফল্য নিশ্চিত করুন।</p>
            <button className="btn btn-secondary">এখনই রেজিস্টার করুন</button>
          </motion.div>
        </div>
      </div>
    </div>
